...
def _looks_unfenced(cp, code) -> bool:
    ...
    # **1行も囲まれていないときだけ弾く。**
    #
    # はじめは「全行が囲まれていること」を求めた。厳しすぎた。Copilot は
    # コードブロックをいくつかに分け、その間に囲まない行を挟むことがある
    # (実測 2026-09-12: 35行のうち7行が繋ぎの行で、`'@ | Set-Content ...` や
    # `python wordcount.py sample.txt` だった)。この書き方まで弾くと、壊れて
    # いないものを止めることになる。
    #
    # 報告された事故は「**丸ごと**囲まれていない」形で、そのときは1行も一致しない。
    # そこを境目にする。
    #
    # 【この線引きで見逃すもの】
    # 囲まれていない繋ぎの行に # や ` や _ があれば、そこは壊れたまま通る。実測で
    # 見た繋ぎの行にはどれも無かったが、無いとは限らない。全部を弾く強さと、
    # 正しいものを止めない使いやすさの折り合いとして、ここを選んでいる。
    # 残った取りこぼしは repair_lost_underscores と、実行が失敗したときの
    # 足踏み検知で拾う。
...
def format_unfenced_report(snippet_id: str) -> str:
    ...
    return "\n".join([
        ...
        "囲みの中は一字一句そのままで構いません。",
        "",
        # ここに `#start <ID> 〜 #end <ID>` と**そのまま書かないこと**。
        # この文面は会話に残り、次の周で応答として読み返される。ID が本物だと
        # 抽出の正規表現に当たり、**この一文自体がスニペットとして拾われる**
        # (実測 2026-09-12: 本文3文字の「 〜 」というスニペットが取れた)。
        f"目印の #start / #end（ID は {snippet_id}）も忘れずに付けてください。",
        "（実測: 囲み直してもらったとき、目印のほうが落ちて拾えなくなった）",
    ])
